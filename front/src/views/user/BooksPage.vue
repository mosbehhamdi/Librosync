    ...
    <ion-content class="ion-padding">
              ...
              <ion-col size="12" size-md="6">
                <div class="search-container">
                  ...
                  >
                    ...
                    {{ bookStore.isLoading ? t('common.status.loading') : t('common.actions.search') }}
                  </ion-button>
                </div>
              </ion-col>
              <ion-col size="12" size-md="6">
                <ion-select
                  :placeholder="t('books.filters.categoryPlaceholder')"
                  @ionChange="onCategoryChange"
                >
                  <ion-select-option value="">{{ t('books.filters.allCategories') }}</ion-select-option>
                  <ion-select-option
      ...
      <ion-card v-else-if="showResults && books.length === 0">
        <ion-card-content class="ion-text-center">
          <ion-text color="medium">
            {{ t('books.search.noResults', { query: searchQuery }) }}
            <template v-if="selectedCategory">
              {{ t('books.search.inCategory', { category: getDeweyCategory(selectedCategory) }) }}
            </template>
          </ion-text>
        </ion-card-content>
      </ion-card>
        ...
        <ion-item v-for="book in books" :key="book.id">
          <ion-label>
            ...
            <p class="book-authors">
              <strong>{{ t('books.fields.authors') }}:</strong> {{ Array.isArray(book.authors) ? book.authors.join(', ') : book.authors }}
            </p>
            <p class="book-category">
              <strong>{{ t('books.fields.category') }}:</strong> {{ getDeweyCategory(book.dewey_category) }}
            </p>
            <p class="book-availability">
              <strong>{{ t('books.fields.available') }}:</strong> {{ book.available_copies }} / {{ book.copies_count }}
            </p>
            <p v-if="book.publisher" class="book-publisher">
              <strong>{{ t('books.fields.publisher') }}:</strong> {{ book.publisher }}
            </p>
            <p v-if="book.waiting_time > 0" class="book-waiting-time">
              <strong>{{ t('books.fields.waitingTime') }}:</strong> {{ t('books.fields.waitingDays', { days: book.waiting_time }) }}
            </p>
          </ion-label>
          <ReserveBookButton
            slot="end"
